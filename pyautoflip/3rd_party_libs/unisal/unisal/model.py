...
class BaseModel(nn.Module):
    ...
    def load_weights(self, directory, name):
        self.load_state_dict(torch.load(directory / f'weights_{name}.pth', map_location=torch.device('cpu')))

    def load_best_weights(self, directory):
        self.load_state_dict(torch.load(directory / f'weights_best.pth', map_location=torch.device('cpu')))

    def load_epoch_checkpoint(self, directory, epoch):
        """Load state_dict from a Trainer checkpoint at a specific epoch"""
        chkpnt = torch.load(directory / f"chkpnt_epoch{epoch:04d}.pth", map_location=torch.device('cpu'))
