...
class Trainer(utils.KwConfigClass):
	...
	def run_inference_frames_fast(self, source, vid_nr, dataset=None, phase=None,
					  smooth_method=None, metrics=None, save_predictions=False,
					  return_predictions=False, seq_len_factor=0.5,
					  random_seed=27, n_aucs_maps=10, auc_portion=1.0,
					  model_domain=None, folder_suffix=None):
	...
	def run_inference_fast(self, source, vid_nr, dataset=None):
		...
		# Select static or dynamic forward pass for Bypass-RNN
		model_kwargs.update({'static': True})
	...
	def run_inference_fast2(self, source, dataset=None):

		# Get the original resolution
		target_size = dataset.target_size_dict[0]
		...
		# Get the data
		frame_seq = dataset.get_all_data()
		frame_idx_array = list(range(len(dataset)))
	...
	def init_unisal_weights(self, source='SALICON'):
		if source is None or source=='SALICON':
			source = 'SALICON'
			self.model.load_best_weights(self.train_dir)
			print(' (unisal init: Best weights loaded)')

		if source in ('MIT1003', 'MIT300'):
			self.model.load_weights(self.train_dir, "ft_mit1003")
			print(' (unisal init: Fine-tuned MIT1003 weights loaded)')
		torch.cuda.empty_cache()
	
	def generate_predictions_from_image_memory(self, images, out_names, out_dir, source=None, **kwargs):
		smaps = []
		with torch.no_grad():
			dataset = data.MemoryImageDataset(images)

			for img_idx in range(len(dataset)):
				pred_seq = self.run_inference_fast('SALICON', img_idx, dataset=dataset, **kwargs)
				...
				# Save prediction as image
				if len(out_names)>0:
					cv2.imwrite(os.path.join(out_dir, out_names[img_idx]), (smap / np.amax(smap) * 255).astype(np.uint8))
		return smaps
		
	def generate_predictions_from_image_memory_nuint8(self, images, out_names, out_dir, source=None, **kwargs):
		smaps = []
		with torch.no_grad():
			dataset = data.MemoryImageDataset(images)
			...
			for img_idx in range(len(dataset)):
				...
				smap = torch.squeeze(smap)
				smap = utils.to_numpy(smap)
				smap = (smap/np.amax(smap)) * 255.0
				smap = smap.astype('uint8')
				smaps.append(smap)
	...
	def generate_predictions_from_image_memory_nuint8_np(self, images, out_names, out_dir, source=None, **kwargs):
		smaps = []
		height = images[0].shape[0]
		width = images[0].shape[1]
		c = len(images)
		smaps = np.zeros((height, width,c), dtype='uint8')
		...
		with torch.no_grad():
			...
			for img_idx in range(len(dataset)):
				...
				smap = utils.to_numpy(smap)
				smap = (smap/np.amax(smap)) * 255.0
				smaps[:,:,img_idx] = smap.astype('uint8')
	...
	def generate_predictions_from_image_memory_nuint8_np_fast(self, images, out_names, out_dir, source=None, **kwargs):
		smaps = []
		height = images[0].shape[0]
		width = images[0].shape[1]
		c = len(images)
		...
		with torch.no_grad():
			dataset = data.MemoryImageDataset(images)
			pred_seq = self.run_inference_fast2('SALICON', dataset=dataset, **kwargs)
			
			# Posporcess prediction
			for img_idx in range(c):
				smap = pred_seq[:, img_idx, ...]
				smap = smap.exp()
				smap = torch.squeeze(smap)
				smap = utils.to_numpy(smap)
				smap = (smap/np.amax(smap)) * 255.0
	...
	def generate_predictions_from_frames_memory(self, images, out_names, out_dir, source=None, **kwargs):
		smaps = []
		with torch.no_grad():
			dataset = data.MemoryFramesDataset(images)

			for img_idx in range(len(dataset)):
				pred_seq = self.run_inference_frames_fast('DHF1K', img_idx, dataset=dataset, **kwargs)
	...
	def load_checkpoint(self, file):
		...
		chkpnt = torch.load(file, map_location=torch.device('cpu'))
