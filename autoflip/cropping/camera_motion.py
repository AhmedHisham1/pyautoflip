...
class CameraMotionHandler:
    ...
    def select_camera_motion_mode(
        ...
        Args:
            ...
            key_frame_indices: Indices of key frames
        ...
        is_talking_head = face_count >= len(key_crop_windows) * 0.7  # 70% of frames have centered windows
        ...
        for i in range(1, len(windows)):
            ...
            if i > 1 and last_direction != 0 and curr_direction != 0 and curr_direction != last_direction:
        ...
        logger.debug(f"Scene analysis: avg_movement={avg_movement:.2f}, max_movement={max_movement}, " +
        ...
        if max_movement < self.motion_threshold * motion_threshold_multiplier:
            logger.debug(f"Selected STATIONARY mode - max movement {max_movement} < threshold " +
                        f"{self.motion_threshold * motion_threshold_multiplier}")
            return CameraMotionMode.STATIONARY
            
        elif consistent_direction and avg_movement < max_movement * 0.7:
            logger.debug(f"Selected PANNING mode - consistent direction with avg movement {avg_movement} " +
                        f"< {max_movement * 0.7}")
            return CameraMotionMode.PANNING
            
        else:
            logger.debug(f"Selected TRACKING mode - inconsistent direction or high movement")
    ...
    def interpolate_crop_windows(
        ...
        if len(key_crop_windows) <= 1:
            ...
            if key_crop_windows:
                crop_window = key_crop_windows[0][0]  # Extract the crop window, not the score
            else:
        ...
        for i, (window, _) in enumerate(key_crop_windows):
            ...
            logger.debug(f"Key frame {key_frame_indices[i]}: x={window[0]}, y={window[1]}")
        ...
        if camera_mode == CameraMotionMode.STATIONARY:
            ...
            for i in sample_indices:
                logger.debug(f"Frame {i}: interpolated position ({x_values[i]}, {y_values[i]})")
            ...
            x_interp = interp1d(key_frame_indices, key_xs, kind='linear', bounds_error=False, fill_value=(key_xs[0], key_xs[-1]))
            y_interp = interp1d(key_frame_indices, key_ys, kind='linear', bounds_error=False, fill_value=(key_ys[0], key_ys[-1]))
            ...
            for i in sample_indices:
                logger.debug(f"Frame {i}: interpolated position ({interp_xs[i]}, {interp_ys[i]})")
            
            # Create crop windows for all frames
            return [(x, y, crop_width, crop_height) for x, y in zip(interp_xs, interp_ys)]
    ...
    def smooth_trajectory(
        ...
        logger.debug(f"Before smoothing - X range: {x_min} to {x_max} (span {x_range}), Y range: {y_min} to {y_max} (span {y_range})")
        ...
        logger.debug(f"After smoothing - X range: {sx_min:.1f} to {sx_max:.1f} (span {sx_range:.1f}), Y range: {sy_min:.1f} to {sy_max:.1f} (span {sy_range:.1f})")
        ...
        for i in sample_indices:
            logger.debug(f"Frame {i}: original ({xs[i]}, {ys[i]}) → smoothed ({smoothed_xs[i]:.1f}, {smoothed_ys[i]:.1f})")
        
        # Create new crop windows with smoothed coordinates
        width = crop_windows[0][2]
        height = crop_windows[0][3]
        
        return [(int(x), int(y), width, height) for x, y in zip(smoothed_xs, smoothed_ys)] 