...
class VideoWriter:
    ...
    def write_frame(self, frame: np.ndarray) -> None:
        ...
        if frame.dtype != np.uint8:
            logger.warning(f"Converting frame from {frame.dtype} to uint8 for VideoWriter compatibility")
        ...
        if current_width != target_width or current_height != target_height:
            logger.warning(f"Resizing frame from {current_width}x{current_height} to {target_width}x{target_height}")
    ...
    def _combine_audio_video(self) -> None:
        ...
        try:
            ...
            cmd = [
                ...
                "-vsync", "passthrough",  # Use passthrough to maintain original timing
            ...
            try:
                ...
                orig_result = subprocess.run(orig_duration_cmd, check=True, capture_output=True, text=True)
                output_result = subprocess.run(output_duration_cmd, check=True, capture_output=True, text=True)
                ...
                if abs(orig_duration - output_duration) > max(1.0, orig_duration * 0.05):
                    logger.warning(f"Output video duration ({output_duration:.2f}s) differs significantly from original ({orig_duration:.2f}s)")
                    logger.warning("This might indicate an issue with audio/video synchronization")
