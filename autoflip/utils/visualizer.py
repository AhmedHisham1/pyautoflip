...
class Visualizer:
    ...
    def _update_visualization(cls) -> None:
        ...
        if latest_frame_pos is not None:
            ...
            cv2.rectangle(canvas, (x-2, y-2), (x+w+2, y+h+2), highlight_color, highlight_thickness)
            
            # Add a "LATEST" indicator
            cv2.putText(canvas, "LATEST", (x+w-80, y+20), font, 0.7, highlight_color, 2)
        ...
        cv2.putText(status_canvas, status, (10, status_y), font, 0.7, status_color, 1, cv2.LINE_AA)
        
        # Add frame count and latest frame info
        frame_count_str = f"Frames: {len(cls._frames)}"
        latest_str = f"Latest: {cls._latest_frame_title}" if cls._latest_frame_title else ""
        cv2.putText(status_canvas, frame_count_str, (canvas_width - 200, status_y), font, 0.7, (200, 200, 200), 1, cv2.LINE_AA)
        
        # Combine main canvas with status bar
        full_canvas = np.zeros((canvas_height + status_height, canvas_width, 3), dtype=np.uint8)
        ...
        if not cls._debug_window_created:
            ...
            window_height = max(cls._initial_window_height, canvas_height + status_height)
    ...
    def _check_key_presses(cls, wait_time: int = 1) -> None:
        ...
        if key != -1:
            ...
            # 'f' key to toggle fullscreen
                ...
                fullscreen_prop = cv2.getWindowProperty(cls._debug_window_name, cv2.WND_PROP_FULLSCREEN)
                # Toggle fullscreen
                if fullscreen_prop == 0:  # Not fullscreen
                    cv2.setWindowProperty(cls._debug_window_name, cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
                    logger.debug("Entered fullscreen mode")
                else:
                    cv2.setWindowProperty(cls._debug_window_name, cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_NORMAL)
    ...
    def clear_frames_with_prefix(cls, prefix: str) -> None:
        ...
        if cls._latest_frame_title is not None and cls._latest_frame_title.startswith(prefix):
            cls._latest_frame_title = None
